...
//std::vector<std::vector<double> > getModalBasis(std::vector<double> zquad, int nq, int np, inCalculateRHSStrongDGEulert P);
...
void GetElementStiffnessMatrixWeakNewBasis_V(int P, 
                                            std::vector<double> wquad, 
                                            double **D, double J, 
                                            std::vector<std::vector<double> > Bmat, 
                                            std::vector<std::vector<double> > Dmat, 
                                            std::vector<double>& StiffMatElem);

std::vector<double> GetGlobalStiffnessMatrixWeakNewBasis_V(std::vector<std::vector<double> > Bmat, 
                                            std::vector<std::vector<double> > Dmat, 
                                            int Nel, int P, std::vector<double> wquad, 
                                            double **D, double *Jac, int **map, int Mdim, 
...
std::vector<double> GetGlobalMassMatrixNew_V(int Nel, int P, std::vector<double> wquad, double *Jac, int **map, int Mdim, std::vector<std::vector<double> > basis);
...
void CalculateRHSWeakDGEuler(int t, BasisPoly* bkeynew, Basis* bkey, int np, int nq, int Nel, int P, std::vector<double> zquad, std::vector<double> wquad, std::vector<double> z, std::string btype, std::string ptype, double **D, double *Jac, int **map, std::vector<std::vector<double> > bc, std::vector<double> X_DG, std::vector<std::vector<double> > U_DG, std::vector<std::vector<double> > &R_DG, double dt, std::vector<std::vector<double> > basis)
{
    ...
    //std::vector<double> StiffnessMatGlobal_V;
    
    
    //StiffnessMatGlobal_V.reserve(Mdim*Mdim);
   
    // for(int i=0;i<Mdim;i++)
    // {
    //     for(int j=0;j<Mdim;j++)
    //     {
    //         StiffnessMatGlobal_V[j*Mdim+i] = StiffnessMatGlobal[i][j];
    //     }
    // }
    ...
    //result.reserve(Mdim * Mdim);
    std::vector<double> StiffnessMatGlobal = GetGlobalStiffnessMatrixWeakNewBasis_V(Bmat, Dmat, Nel, P, wquad, D, Jac, map, Mdim, Bmat);

    for(int po=0;po<(Mdim);po++)
    {
        for(int q=0;q<(Mdim);q++)
        {
            int entry = po*Mdim+q;
            //std::cout << result[entry] << " " << StiffnessMatGlobal[po][q] << std::endl;
        }
    ...
    // for (int i = 0; i < Mdim; ++i) {
    //     for (int j = 0; j < Mdim; ++j) {
    //         result.push_back(StiffnessMatGlobal[i][j]);
    //     }
    // }
    ...
    std::vector<double> tmp0(Mdim,0);
    std::vector<double> tmp1(Mdim,0);
    std::vector<double> tmp2(Mdim,0);

    // /dgemv_(&TRANS,&Mdim,&Mdim,&ONE_DOUBLE,StiffnessMatGlobal[0],&Mdim,F_DG_coeff[0].data(),&ONE_INT,&ZERO_DOUBLE,tmp0,&ONE_INT);

    dgemv_(&TRANS,&Mdim,&Mdim,&ONE_DOUBLE,StiffnessMatGlobal.data(),&Mdim,F_DG_coeff[0].data(),&ONE_INT,&ZERO_DOUBLE,tmp0.data(),&ONE_INT);
    dgemv_(&TRANS,&Mdim,&Mdim,&ONE_DOUBLE,StiffnessMatGlobal.data(),&Mdim,F_DG_coeff[1].data(),&ONE_INT,&ZERO_DOUBLE,tmp1.data(),&ONE_INT);
    dgemv_(&TRANS,&Mdim,&Mdim,&ONE_DOUBLE,StiffnessMatGlobal.data(),&Mdim,F_DG_coeff[2].data(),&ONE_INT,&ZERO_DOUBLE,tmp2.data(),&ONE_INT);
    ...
    std::vector<double> MassMatGlobal0 = GetGlobalMassMatrixNew_V(Nel, P, wquad, Jac, map, Mdim, Bmat);
    std::vector<double> MassMatGlobal1 = GetGlobalMassMatrixNew_V(Nel, P, wquad, Jac, map, Mdim, Bmat);
    std::vector<double> MassMatGlobal2 = GetGlobalMassMatrixNew_V(Nel, P, wquad, Jac, map, Mdim, Bmat);
    ...
    dgetrf_(&Mdim, &Mdim, MassMatGlobal0.data(), &Mdim, ipiv, &INFO);
    dgetrf_(&Mdim, &Mdim, MassMatGlobal1.data(), &Mdim, ipiv, &INFO);
    dgetrf_(&Mdim, &Mdim, MassMatGlobal2.data(), &Mdim, ipiv, &INFO);

    dgetrs_(&TRANS, &Mdim, &NRHS, MassMatGlobal0.data(), &Mdim, ipiv, Ucoeff_eq0.data(), &Mdim, &INFO);
    dgetrs_(&TRANS, &Mdim, &NRHS, MassMatGlobal1.data(), &Mdim, ipiv, Ucoeff_eq1.data(), &Mdim, &INFO);
    dgetrs_(&TRANS, &Mdim, &NRHS, MassMatGlobal2.data(), &Mdim, ipiv, Ucoeff_eq2.data(), &Mdim, &INFO);
...
{
    double **StiffnessElem = dmatrix(P+1);
    std::vector<double> StiffnessElem_V;
    StiffnessElem_V.reserve((P+1)*(P+1));
    std::vector<double> StiffnessGlobal_V(Mdim*Mdim,0);
    // Construct global Mass matrix.
    for(int i=0;i<Mdim;i++)
    {
        for(int j=0;j<Mdim;j++){
    ...
    for(int eln=0;eln<Nel;eln++)
    {
        ...
        GetElementStiffnessMatrixWeakNewBasis_V(P, wquad, D, Jac[eln], Bmat, Dmat, StiffnessElem_V);
        ...
        for(int a=0;a<P+1;a++)
        {
            for(int b=0;b<P+1;b++)
            {
                // Assemble the global mass matrix;
                StiffnessGlobal[map[eln][a]][map[eln][b]] = StiffnessGlobal[map[eln][a]][map[eln][b]] + StiffnessElem[a][b];
                StiffnessGlobal_V[map[eln][a]*Mdim+map[eln][b]] = StiffnessGlobal_V[map[eln][a]*Mdim+map[eln][b]] = StiffnessElem_V[a*(P+1)+b];
            }
        }   
    }
}
...
std::vector<double> GetGlobalStiffnessMatrixWeakNewBasis_V(std::vector<std::vector<double> > Bmat, std::vector<std::vector<double> > Dmat, int Nel, int P, std::vector<double> wquad, double **D, double *Jac, int **map, int Mdim, std::vector<std::vector<double> > basis)
{
    ...
    //     for(int j=0;j<Mdim;j++)
    //     {
    //         std::cout << "error " << StiffnessGlobal[i][j]-StiffnessGlobal_V[i*Mdim+j] << std::endl;
    //     }
    // }
                                            ...
                                            std::vector<double>& StiffMatElem)
{
    ...
    {   
        ...
        for(int j=0;j<P+1;j++)
        {
            ...
            StiffMatElem.push_back(J*integr(nq, wquad.data(), dphi1.data(), phi2.data()));
            // StiffMatElem[i][j] = J*integr(nq, wquad.data(), basis_diff_update[i].data(), basis_update[j].data());
        }
    }
}
...
std::vector<double> GetGlobalMassMatrixNew_V(int Nel, int P, std::vector<double> wquad, double *Jac, int **map, int Mdim, std::vector<std::vector<double> > basis)
{
    // double **MassMatElem = dmatrix(P+1);
    // Construct global Mass matrix.
    std::vector<double> MassMatGlobal_V(Mdim*Mdim,0);
    // for(int i=0;i<Mdim;i++)
    // {
    //     for(int j=0;j<Mdim;j++){
    //         MassMatGlobal[i][j] = 0;
    ...
    for(int eln=0;eln<Nel;eln++)
    {
        ...
        for(int a=0;a<P+1;a++)
        {
            for(int b=0;b<P+1;b++)
            {
                ...
                MassMatGlobal_V[map[eln][a]*Mdim+map[eln][b]] = MassMatGlobal_V[map[eln][a]*Mdim+map[eln][b]] + MassMatElem[a*(P+1)+b];
                // MassMatGlobal[map[eln][a]][map[eln][b]] = MassMatGlobal[map[eln][a]][map[eln][b]] + MassMatElem[a*(P+1)+b];//MassMatElem[a][b];
            }
        }
    }

    return MassMatGlobal_V;
}
