...
{
    int solSize   = NVAR_GPU * gpu.totalDOF;
    ...
    ADJ_CUDA_CHECK(cudaMalloc(&adj.d_adjSurfQuad, solSize * sizeof(double)));

    ADJ_CUDA_CHECK(cudaMalloc(&adj.d_dJdU, solSize * sizeof(double)));
    ...
    ADJ_CUDA_CHECK(cudaMemset(adj.d_dJdU, 0, solSize * sizeof(double)));
...
void adjointGpuFree(AdjointGPUData& adj)
{
    ...
    cudaFree(adj.d_adjR);
    cudaFree(adj.d_psiCoeff);    cudaFree(adj.d_adjVolCoeff);
...
void adjointComputeRHS(AdjointGPUData& adj, const GPUSolverData& gpu, bool usePsiTmp)
{
    ...
    // Step 1b: Compute adjoint smoothness sensor → d_adjEpsilon
    //   The AV scale must respect the diffusive CFL limit:
    //     dt < h^2 / (2 * eps * (2P+1)^2),  eps = scale * h * smax / (2P)
    //   With convective dt = CFL * h / (smax * (2P+1)):
    //     scale_max = P / (CFL * (2P+1))
    if (gpu.useAV) {
        int bkS = 256;
        ...
        int P = P1 - 1;
        double adjAVscale = std::min((double)gpu.AVscale,
        ...
        adjointSensorKernel<<<gdS, bkS>>>(
            ...
            nE, P1, nmodes, nqVol,
            gpu.AVs0, gpu.AVkappa, smaxRef, adjAVscale);
    }
    ...
    // Step 6: Add source term (already unweighted from adjointComputeObjectiveGradient)
...
void adjointResidualNormPerVar(AdjointGPUData& adj, int totalDOF, double norms[4])
{
    ...
    int bk = 256;
    int gd = std::min((totalDOF + bk - 1) / bk, 1024);
    adjResNormPerVarKernel<<<gd, bk, 4 * bk * sizeof(double)>>>(
        adj.d_adjR, adj.d_normBuf, totalDOF);
